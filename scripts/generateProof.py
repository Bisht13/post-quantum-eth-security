...
# Read blob.bin and save the binary data to blob.txt
with open(os.path.join(args.output_folder, f'blob.bin'), 'rb') as f:
    data = f.read()
    with open(os.path.join(args.output_folder, f'blob.txt'), 'w') as f:
        # Save the data as hex string
        f.write(data.hex())

# Delete blob.bin
os.remove(os.path.join(args.output_folder, f'blob.bin'))
