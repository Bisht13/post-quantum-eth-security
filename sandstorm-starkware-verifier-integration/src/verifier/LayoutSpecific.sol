    ...
    CairoVerifierContract
{
    ...
    function validateBuiltinPointers(
        uint256 initialAddress,
        uint256 stopAddress,
        uint256 builtinRatio,
        uint256 cellsPerInstance,
        uint256 nSteps,
        string memory builtinName
    ) internal pure {
        require(
            initialAddress < 2 ** 64,
            string(
                abi.encodePacked("Out of range ", builtinName, " begin_addr.")
            )
        );
        uint256 maxStopPtr = initialAddress +
            cellsPerInstance *
            safeDiv(nSteps, builtinRatio);
        require(
    ...
    function layoutSpecificInit(
        ...
        validateBuiltinPointers(
            ...
            publicInput[OFFSET_PEDERSEN_STOP_PTR],
        ...
        validateBuiltinPointers(
            ...
            publicInput[OFFSET_RANGE_CHECK_STOP_PTR],
            RC_BUILTIN_RATIO,
            1,
            nSteps,
            "range_check"
        ...
        validateBuiltinPointers(
            ...
            publicInput[OFFSET_BITWISE_STOP_ADDR],
            BITWISE__RATIO,
            5,
            nSteps,
            "bitwise"
    ...
    function prepareForOodsCheck(uint256[] memory ctx) internal view {
        ...
        ctx[MM_PERIODIC_COLUMN__PEDERSEN__POINTS__X] = pedersenPointsX.compute(
            zPointPowPedersen
        );
        ctx[MM_PERIODIC_COLUMN__PEDERSEN__POINTS__Y] = pedersenPointsY.compute(
            zPointPowPedersen
        );
        ...
        ctx[MM_DILUTED_CHECK__INTERACTION_ALPHA] = ctx[
            MM_INTERACTION_ELEMENTS + 5
        ];
    ...
    */
    function computeDilutedCumulativeValue(
        uint256[] memory ctx
