...
contract FriLayer is MerkleVerifier, FriTransform {
    ...
    */
    function gatherCosetInputs(
        ...
        internal
        pure
        returns (uint256 newFriQueueHead, uint256 cosetIdx, uint256 cosetOffset)
    {
        assembly {
            ...
            for {
                ...
                mstore(
                    ...
                    mod(mload(fieldElementPtr), K_MODULUS)
                )
    ...
    */
    function initFriGroups(uint256 friCtx) internal view {
        ...
        uint256 friHalfInvGroupPtr = friCtx +
        ...
        for (uint256 i = 1; i < halfCosetSize; i++) {
            ...
            assembly {
                ...
                mstore(
                    add(friGroupPtr, add(mul(idx, 0x40), 0x20)),
                    sub(K_MODULUS, lastVal)
                )
            }
        }
    }
