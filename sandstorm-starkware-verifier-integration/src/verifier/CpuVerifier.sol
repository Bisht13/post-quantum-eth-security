    ...
    PublicMemoryOffsets
{
    ...
    constructor(
        ...
        public
        StarkVerifier(numSecurityBits_, minProofOfWorkBits_, oodsContract)
    {
    ...
    function getNColumnsInComposition()
        internal
        pure
        override
        returns (uint256)
    ...
    function getNInteractionElements()
        internal
        pure
        override
        returns (uint256)
        ...
        returns (uint256[] memory ctx, uint256 logTraceLength)
    {
        ...
        // Layout.
        require(
            publicInput[OFFSET_LAYOUT_CODE] == LAYOUT_CODE,
...

        {
            ...
            for (uint256 page = 0; page < ctx[MM_N_PUBLIC_MEM_PAGES]; page++) {
                ...
                require(
                    n_page_entries < 2 ** 30,
                    "Too many public memory entries in one page."
                );
        ...
        uint256 expectedPublicInputLength = getPublicInputLength(
            ctx[MM_N_PUBLIC_MEM_PAGES]
        );
        require(
        ...
        assembly {
            ...
            mstore(
                add(ctx, mul(add(lmmPublicInputPtr, 1), 0x20)),
    ...
    function getPublicInputHash(
        uint256[] memory publicInput
    ) internal pure override returns (bytes32 publicInputHash) {
    ...
    */
    function computePublicMemoryQuotient(
        ...
        require(
            nValues <= publicMemorySize,
            "Number of values of public memory is too large."
        );
        ...
        assembly {
            let paddingAddrPtr := add(
                publicInputPtr,
                mul(0x20, OFFSET_PUBLIC_MEMORY_PADDING_ADDR)
            )
            paddingAddr := mload(paddingAddrPtr)
            paddingValue := mload(add(paddingAddrPtr, 0x20))
        }
        uint256 hash_first_address_value = fadd(
            paddingAddr,
            fmul(paddingValue, alpha)
        );
    ...
    */
    function verifyMemoryPageFacts(uint256[] memory ctx) private view {
        ...
        for (uint256 page = 0; page < nPublicMemoryPages; page++) {
            ...
            uint256 memoryHashPtr = ctx[MM_PUBLIC_INPUT_PTR] +
                getOffsetPageHash(page) *
                0x20;
            uint256 memoryHash;
            ...
            uint256 pageSizePtr = ctx[MM_PUBLIC_INPUT_PTR] +
                getOffsetPageSize(page) *
                0x20;
            uint256 pageSize;
            ...
            uint256 pageAddr = 0;
            if (page > 0) {
                uint256 pageAddrPtr = ctx[MM_PUBLIC_INPUT_PTR] +
                    getOffsetPageAddr(page) *
                    0x20;
                assembly {
    ...
    */
    function oodsConsistencyCheck(uint256[] memory ctx) internal view override {
        ...
        ctx[MM_MEMORY__MULTI_COLUMN_PERM__PERM__INTERACTION_ELM] = ctx[
            MM_INTERACTION_ELEMENTS
        ];
        ctx[MM_MEMORY__MULTI_COLUMN_PERM__HASH_INTERACTION_ELM0] = ctx[
            MM_INTERACTION_ELEMENTS + 1
        ];
